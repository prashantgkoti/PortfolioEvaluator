...
async function handleUpload(){
  ...
  if(!fileInput.files.length){ statusEl.textContent = 'Choose a PDF first.'; return; }
  ...
  try{
    const result = await api('/api/cas/upload', {method:'POST', body: form});
    statusEl.textContent = `Parsed ${result.holdings_count} holdings.`;
    if(result.warnings && result.warnings.length){
      warningsEl.innerHTML = result.warnings.map(w=>`<div class="callout watch" style="margin-top:10px;">${w}</div>`).join('');
    }
    await loadPortfolio(false);
  }catch(e){
...
async function loadPortfolio(refresh){
  ...
  document.getElementById('uploadCard').style.display = hasHoldings ? 'none' : 'block';
  ...
  renderRupeeBuckets(data.rupee_buckets, data.total_value);
...
async function loadTrendAndNps(data){
  ...
  const nps = await api('/api/nps');
  if(nps && nps.value){
    // Repurpose the growth-KPI subtitle area isn't ideal; append an NPS line to equity/MF sub instead
    const sub = document.getElementById('kpiEquityMfSub');
    sub.innerHTML += `<br>NPS: ${fmtCompact(nps.value)} · XIRR ${nps.xirr}%`;
  }
}
...
function renderRupeeBuckets(buckets, total){
  const labels = {cas:'CAS upload', manual_us:'US market trades', unlisted:'Unlisted shares', gold:'Gold', manual_other:'Other assets'};
  document.getElementById('rupeeBucketBody').innerHTML = buckets
    .sort((a,b)=>b.value-a.value)
    .map(b=>`<tr><td>${labels[b.source]||b.source}</td><td class="num">${indianGrouped(b.value)}</td><td class="num">${total?(b.value/total*100).toFixed(1):0}%</td></tr>`)
    .join('');
}
...
function renderEquityTable(holdings){
  ...
  document.getElementById('equitySub').textContent = `${stocks.length} positions with a live balance, totalling ${fmtCompact(total)}. NSDL/CDSL CAS statements don't track purchase cost for demat equities.`;
  document.getElementById('equityBody').innerHTML = stocks.map(h=>{
    ...
    return `<tr><td>${h.name||'—'}</td><td>${h.symbol||'—'}</td><td class="num">${h.quantity!=null?h.quantity.toLocaleString('en-IN'):'—'}</td>
      ...
      <td class="num">${indianGrouped(h.value_inr)}</td><td class="num">${pct}%</td></tr>`;
  }).join('');
}
